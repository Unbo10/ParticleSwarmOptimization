...
class SelectMenu():
    ...
    def display(self):
        ...
        if len(self.__optimization_history) == 0:
            ...
            self.__parent_frame.bind("<MouseWheel>", self.__scroll_mouse_wheel)
            self.__parent_frame.bind("<Button-4>", self.__scroll_mouse_wheel)
            self.__parent_frame.bind("<Button-5>", self.__scroll_mouse_wheel)
            ...
            self.__canvas.create_window((0, 0), window=self.__parent_frame, anchor="nw", width=self.__parent_frame_width, height=self.__parent_frame_height) # * Creates a window inside the current tk.Tk() window, so it only displays a certain portion of the widgets contained inside.
